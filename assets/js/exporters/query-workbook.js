...
function buildQueryWorkbook(model, filters, priceYear){
  ...
  /* ---------- Sheet 1：同分組項目明細 ----------
     ...
     欄位：分組代碼(A) CODE(B) 中文名稱(C) 藥品名稱(D) ATC7碼(E) ATC名稱(F)
           支付價(G) 含量(H) 含量單位(I) 規格量(J) 規格單位(K) 不良品暫停支付註記(L)
           分組名稱(M) 劑型(N) 藥商名稱(O)
           YR1-3 申報量(P-U) YR1-3 申報金額  ← 數量在前，金額在後（同分頁慣例）
  ...
  var H1 = ['分組代碼', 'CODE', '中文名稱', '藥品名稱', 'ATC7碼', 'ATC名稱',
            '支付價' + priceYear, '含量', '含量單位', '規格量', '規格單位',
            '不良品暫停支付註記',
            '分組名稱', '劑型', '藥商名稱',
            YR1 + '年申報量', YR1 + '年申報金額',
            YR2 + '年申報量', YR2 + '年申報金額',
            YR3 + '年申報量', YR3 + '年申報金額'];
  var NCOL1 = H1.length;  /* 21 */
  var ST1 = H1.map(styleForHeader);
  ST1[0] = ST1[1] = ST1[4] = XS.BODY;          /* 分組代碼/CODE/ATC7碼 文字 */
  ST1[7] = ST1[8] = ST1[9] = ST1[10] = XS.BODY; /* 含量/含量單位/規格量/規格單位 文字 */
  ...
  function subtotal1(gcode){
    ...
    row[12] = xcell(g ? g.name : '', XS.SUB);  /* 分組名稱 */
    row[15] = xcell(Math.round(gQty1[0]), XS.SUBINT);
    row[16] = xcell(Math.round(gAmt1[0]), XS.SUBINT);
    row[17] = xcell(Math.round(gQty1[1]), XS.SUBINT);
    row[18] = xcell(Math.round(gAmt1[1]), XS.SUBINT);
    row[19] = xcell(Math.round(gQty1[2]), XS.SUBINT);
    row[20] = xcell(Math.round(gAmt1[2]), XS.SUBINT);
    return row;
  }
  ...
  if (nItems1){
    ...
    tot1[15] = xcell(Math.round(tQty1[0]), XS.SUBINT); tot1[16] = xcell(Math.round(tAmt1[0]), XS.SUBINT);
    tot1[17] = xcell(Math.round(tQty1[1]), XS.SUBINT); tot1[18] = xcell(Math.round(tAmt1[1]), XS.SUBINT);
    tot1[19] = xcell(Math.round(tQty1[2]), XS.SUBINT); tot1[20] = xcell(Math.round(tAmt1[2]), XS.SUBINT);
    s1.push(tot1);
  }
